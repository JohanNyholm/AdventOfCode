...
def run_program(program, start_index=0):
    ...
    if op == 1:
        ...
        program[result_pos] = program[operand_pos_1] * program[operand_pos_2]
    run_program(program, start_index+4)


def run(base_program, noun, verb):
...
# unique_indices = set({ i: v for i, v in enumerate(p[:100])}.keys())
