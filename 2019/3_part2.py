...
def translate_move(move):
    ...
    x_motion = num_steps * {'R': 1, 'L': -1, 'U': 0, 'D': 0}[direction]
    y_motion = num_steps * {'R': 0, 'L': 0, 'U': 1, 'D': -1}[direction]
...
def walk_path(path):
    ...
    current_pos = (0, 0, 0)
    for move in path:
        current_pos = walk(visited, current_pos, move)
    return visited

path1 = walk_path(w1)
path1.pop((0,0))
path1_cords = set(path1.keys())
path2 = walk_path(w2)
path2.pop((0,0))
path2_cords = set(path2.keys())
# print(path1)
# print(path2)
common_cords = list(path1_cords.intersection(path2_cords))
# print(common_cords)
# common_cord_travel_distances = [(cord, path1[cord] + path2[cord]) for cord in common_cords]
common_cord_travel_distances = [path1[cord] + path2[cord] for cord in common_cords]
print(min(common_cord_travel_distances))
# sorted_distances = sorted(common_cord_travel_distances, key=lambda cord_n_t_dist: cord_n_t_dist[1])
# # sorted_distances = sorted(common_cords, key=lambda cord: abs(sum(cord)))
# print (abs(sum(sorted_distances[1])))