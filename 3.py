...
def walk(visited, from_pos, move):
    ...
    (current_x, current_y) = from_pos
    new_cords = []
    if x_motion > 0:
        new_cords = [(x, current_y) for x in range(current_x, current_x + x_motion + 1)]
    elif x_motion < 0:
        new_cords = [(x, current_y) for x in range(current_x, current_x + x_motion - 1, -1)]
    elif y_motion > 0:
        new_cords = [(current_x, y) for y in range(current_y, current_y + y_motion + 1)]
    elif y_motion < 0:
        new_cords = [(current_x, y) for y in range(current_y, current_y+y_motion-1, -1)]
    visited.update(new_cords)
    new_point = (current_x + x_motion, current_y + y_motion)
    return new_point
...
path2 = walk_path(w2)
path2.remove((0,0))
common_cords = list(path1.intersection(path2))
sorted_distances = sorted(common_cords, key=lambda cord: abs(sum(cord)))
print (abs(sum(sorted_distances[0])))