...
def solve(draws, boards):
    ...
    while True:
        ...
        if len(boards) == 0:
            # assume we only had one, pre draw
            looser_board = boards_pre_draw[0]
            return sum(
                sum(row)
                for row in looser_board['cols']  # can collect from either rows or cols, same thing
            ) * draw
